...
> Lote opcional no consumo, renovação de sessão sem deslogar, cadastro de fabricantes como entidade, categorias-folha em cascata e número de série automático no patrimônio. Entregue e validado via betas `1.14.0-beta-01..05`.

### Adicionado
- **Número de série automático na entrada de patrimônio** ([patrimony.js](api/inventory/patrimony.js), [patrimony-entry.html](public/inventory/patrimony-entry.html)): o número de série deixa de ser obrigatório. Quando deixado **em branco**, o sistema gera `<código do item>-NN` sequencial por item — `NN` começa em `01` e alarga para 3+ dígitos após 99 —, sem reusar números (mesmo de unidades apagadas) nem colidir com séries informadas no mesmo formulário. A tela exibe os números gerados após salvar. Sem mudança de schema (usa `inv_serial_units`).
...
### Notas
- [_layout.js:5](public/inventory/_layout.js#L5) bump para `v1.14.0` (estável).
- **Migrações desta release** (aplicar no Supabase, em ordem): [114-controls-lot-manual.sql](database/inventory-refactor/114-controls-lot-manual.sql), [115-fabricantes.sql](database/inventory-refactor/115-fabricantes.sql). As demais funcionalidades (sessão, categorias, nº de série) não exigem migração.
